...
func main() {
	cmd := exec.Command("godoc",
		"-url", "/pkg/github.com/maxence-charriere/go-app/v9/pkg/app")
	out, err := cmd.Output()
	if err != nil {
...
func normalizeNode(n *html.Node) {
	if n.Type == html.ElementNode {
		...
		for i, a := range n.Attr {
			...
			switch {
			case strings.HasPrefix(u.Path, "/src/github.com/maxence-charriere/go-app/v9"):
				u.RawQuery = ""
				u.Path = strings.TrimPrefix(u.Path, "/src/github.com/maxence-charriere/go-app/v9")

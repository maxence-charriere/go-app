...
type Handler struct {
	...
	// WithCacheBustingQueryParam indicates whether to append a cache-busting
	// query parameter to resource URLs. Defaults to false.
	// Version is used if true.  i.e. ?v={{.Version}}.
	WithCacheBustingQueryParam bool
...
// resolveWithVersion resolves a resource path and optionally appends version query parameter
func (h *Handler) resolveWithVersion(path string) string {
	resolved := h.Resources.Resolve(path)
	if h.WithCacheBustingQueryParam && h.Version != "" {
		// Only add version to non-remote resources
		if !isRemoteLocation(resolved) {
			separator := "?"
			if strings.Contains(resolved, "?") {
				separator = "&"
			}
			resolved = resolved + separator + "v=" + h.Version
		}
	}
	return resolved
}
...
func (h *Handler) makeAppJS() []byte {
	...
	s = strings.ReplaceAll(s, "{{.Wasm}}", h.resolveWithVersion("/web/app.wasm"))
	...
	s = strings.ReplaceAll(s, "{{.WorkerJS}}", h.resolveWithVersion("/app-worker.js"))
	return []byte(s)
}
...
func (h *Handler) makeAppWorkerJS() []byte {
	...
	for k := range resources {
		resourcesTocache = append(resourcesTocache, h.resolveWithVersion(k))
	}
...
func (h *Handler) makeManifestJSON() []byte {
	...
	if iconURL := h.resolveWithVersion(h.Icon.Maskable); iconURL != "" {
	...
	if iconURL := h.resolveWithVersion(h.Icon.SVG); iconURL != "" {
		icons = append(icons, manifestIcon{
			Src:   iconURL,
			Type:  "image/svg+xml",
			Sizes: "any",
	...
	if iconURL := h.resolveWithVersion(h.Icon.Large); iconURL != "" {
		icons = append(icons, manifestIcon{
			Src:   iconURL,
			Type:  "image/png",
			Sizes: "512x512",
	...
	icons = append(icons, manifestIcon{
		Src:   h.resolveWithVersion(h.Icon.Default),
		Type:  "image/png",
		Sizes: "192x192",
	})
			...
			Head().Body(
				...
				Range(h.Fonts).Slice(func(i int) UI {
					if resource := parseHTTPResource(h.Fonts[i]); resource.URL != "" {
						resource.URL = h.resolveWithVersion(resource.URL)
				...
				Link().
					Rel("icon").
					Href(h.resolveWithVersion(icon)),
				Link().
					Rel("apple-touch-icon").
					Href(h.resolveWithVersion(h.Icon.Maskable)),
				Link().
					Rel("manifest").
					Href(h.resolveWithVersion("/manifest.webmanifest")),
				...
				Script().
					Defer(true).
					Src(h.resolveWithVersion("/wasm_exec.js")),
				Script().
					Defer(true).
					Src(h.resolveWithVersion("/app.js")),
